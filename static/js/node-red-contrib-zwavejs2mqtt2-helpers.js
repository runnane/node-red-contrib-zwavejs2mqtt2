function z2m_getItemList(nodeItem, selectedItemElementName, options = {}) {
    ...
    function z2m_updateItemList(controller, selectedItemElement, itemName, refresh = false) {
        ...
        if (controller) {
            ...
            $.getJSON('zwavejs2mqtt2/getDevices', {
                ...
                .done(function (data, textStatus, jqXHR) {
                    try {
                       ...
                       // var groups = data[1];
                        ...
                        //groups
                        /*
                        groupHtml = $('<optgroup/>', {label: RED._("node-red-contrib-zwavejs2mqtt2/in:multiselect.groups")});
                        ...
                        $.each(groups, function(index, value) {
                            ...
                            $('<option value="' + value.ID + '" data-friendly_name="'+value.name+'">' + value.name + text + '</option>').appendTo(groupHtml);
                        });
                        */
                        //devices
                        groupHtml = $('<optgroup/>', {label: RED._("node-red-contrib-zwavejs2mqtt2/in:multiselect.devices")});
                        groupHtml.appendTo(selectedItemElement);
                        $.each(devices, function(index, value) {
                         ...
                         //   }
                            $('<option value="' + value.id + '" data-friendly_name="'+value.name+'">' + value.name + model + '</option>').appendTo(groupHtml);
                        });
...
function z2m_getItemStateList(nodeItem, selectedItemElementName, options = {}) {
    ...
    function z2m_updateItemStateList(controller, selectedItemElement, itemName) {
        ...
        if (controller && uniqueId) {
            $.getJSON('zwavejs2mqtt2/getLastStateById', {
                controllerID: controller.id,
                device_id:uniqueId
            })
                .done(function (data, textStatus, jqXHR) {
                    try {
                        selectedItemElement.html('<option value="0">'+ RED._("node-red-contrib-zwavejs2mqtt2/in:multiselect.complete_payload")+'</option>');

                        var groupHtml = '';

                        if (data[0] && Object.keys(data[0]).length) {
                            groupHtml = $('<optgroup/>', {label: RED._("node-red-contrib-zwavejs2mqtt2/in:multiselect.zwavejs2mqtt2")});
                            groupHtml.appendTo(selectedItemElement);
